...
// Initialize WebTransport polyfill before importing @moq/lite
const wt = await import("@fails-components/webtransport");
// @ts-ignore - polyfill global WebTransport
...
async function connect(
  ...
  const options: Moq.Connection.Options = {};
...
async function testSubscribeError(
  ...
  const broadcast = conn.consume(
    Moq.Path.from("nonexistent/namespace")
  );
  ...
  try {
    ...
    if (
      e.message?.includes("unexpected success")
    ) {
      throw e;
    }
...
async function testAnnounceSubscribe(
  ...
  const subBroadcast = subConn.consume(
    Moq.Path.from(TEST_NAMESPACE)
  );
...
tap("TAP version 14");
tap("# moq-dev-js-test-client v0.1.0");
...
for (let i = 0; i < tests.length; i++) {
  ...
  try {
    ...
    printFailureDiagnostics(durationMs, e.message || String(e));
  }
}
